...
const ModelLive = ModelRuntime.layerConfig({
  model: { provider: Config.NonEmptyString("PROVIDER"), id: Config.NonEmptyString("MODEL") },
...
const RunnerLive = ClusterSessions.runnerLayerConfig(Config.all({
  cwd: Config.NonEmptyString("CWD"),
  keyPrefix: Config.NonEmptyString("PREFIX")
}).pipe(Config.map((options) => ({
  ...options, configure: () => ({ noTools: "all" as const })
}))))
...
const DiscoveryLive = ResourceLoader.layerConfig(Config.all({
  agentDir: Config.NonEmptyString("CWD"),
  systemPrompt: Config.NonEmptyString("SYSTEM_PROMPT")
}).pipe(Config.map((options) => ({
  ...options, settings: {}, noExtensions: true, noSkills: true,
...
const provide = <A, E, R>(effect: Effect.Effect<A, E, R>) => effect.pipe(
  Effect.scoped, Effect.provide(dependencies)
)
...
it.effect("config layers resolve lazily against each build's ConfigProvider", () => provide(Effect.gen(function*() {
  ...
  }).pipe(Effect.scoped, Effect.provide(ModelLive))
  const first = yield* inspect.pipe(Effect.provide(ConfigProvider.layer(provider({ SYSTEM_PROMPT: "First" }))))
  const second = yield* inspect.pipe(Effect.provide(ConfigProvider.layer(provider({ SYSTEM_PROMPT: "Second" }))))
  it.expect(first).toBe("First")
  it.expect(second).toBe("Second")
})))

it.effect("resource discovery accepts a complete Config of options", () => provide(Effect.gen(function*() {
  const { cwd, provider } = yield* fixture
  ...
  }).pipe(Effect.scoped, Effect.provide(DiscoveryLive), Effect.provide(ConfigProvider.layer(provider())))
})))
...
it.effect("local layerConfig composes model, resources, storage, and configured pool options", () => provide(
  Effect.gen(function*() {
    const { provider } = yield* fixture
    yield* inspectSession(Session.Id.make("config-pool")).pipe(
      Effect.scoped, Effect.provide(PoolLive), Effect.provide(ConfigProvider.layer(provider()))
    )
  })
))

// Real SDK filesystem I/O can trigger cluster retries; those need a live clock.
it.live("runnerLayerConfig serves the public cluster client with configured storage options", () => provide(
  Effect.gen(function*() {
    const { provider } = yield* fixture
    yield* inspectSession(Session.Id.make("config-cluster")).pipe(
      Effect.scoped, Effect.provide(ClusterLive), Effect.provide(ConfigProvider.layer(provider()))
    )
  })
))
...
const expectConfigError = <A, R>(live: Layer.Layer<A, Config.ConfigError, R>) => Layer.build(live).pipe(
  Effect.flip,
  Effect.tap((error) => Effect.sync(() => { it.expect(error._tag).toBe("ConfigError") }))
)

it.effect("all config constructors retain typed configuration failures", () => provide(Effect.gen(function*() {
...
it.effect("missing model configuration fails at layer build, before any session is opened", () => provide(
  Effect.gen(function*() {
    const { provider } = yield* fixture
