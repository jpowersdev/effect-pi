...
See [Direct.ts](../examples/Direct.ts).
...
See [Local.ts](../examples/Local.ts).
...
The [cluster example](../examples/README.md#cluster-two-processes) includes socket transport, runner discovery, and SQL storage. SQLite supports the same-machine demo. Across machines, use a shared database for coordination and session documents; do not put SQLite on a network filesystem.
